addLayer("m", {
    ...
    values: {
        ...
        manifoldChal9_productionRoot() {
            if (!inChallenge("m", 33)) return 1

            for (const id of [5, 4, 3, 2, 1, 0]) {
                if (player.q.generators[id].purchased.gt(0)) return (id + 1)
            }
            return 1
        },

        manifoldChal9_rootModifier() {
            if (!inChallenge("m", 33)) return new Decimal(1)

            let value = new Decimal(0)

            for (const id of [0, 1, 2, 3, 4, 5, 6, 7]) {
                value = value.plus(player.q.generators[id].phases.clampMin(1).log10())
            }

            return value.plus(1).clampMin(1)
        },

        manifoldChal9_divisor() {
            if (!inChallenge("m", 33)) return new Decimal(1)

            let value = new Decimal(0)

            for (const id of [0, 1, 2, 3, 4, 5, 6, 7]) {
                value = value.plus(getBuyableAmount("q", "10" + id))
            }
    ...
    update(diff) {
        ...
        player.m.manifoldConversionRate = new Decimal(0.5).plus(buyableEffect("em", 11))
    ...
    tabFormat: {
        ...
        "Exotic Matter": {
            embedLayer: "em",
            unlocked() {
                return challengeCompletions("m", 33) >= 1
            },
            buttonStyle: {
                "margin-top":"15px"
            },   
