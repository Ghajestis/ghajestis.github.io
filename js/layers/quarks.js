addLayer("q", {
    ...
    startData() { return {
        ...
        commonGeneratorMultiplier: new Decimal(1)
    }},
    color: "#a8a8a8",
    ...
    update(diff) {
        ...
        player.q.atomStrength = new Decimal(2)
        player.q.atomMult = [
            player.q.atomStrength.pow(player.q.atoms.sub(0)),
            player.q.atomStrength.pow(player.q.atoms.sub(1)),
            player.q.atomStrength.pow(player.q.atoms.sub(2)),
            player.q.atomStrength.pow(player.q.atoms.sub(3)),
            player.q.atomStrength.pow(player.q.atoms.sub(4)),
            player.q.atomStrength.pow(player.q.atoms.sub(5)),
            player.q.atomStrength.pow(player.q.atoms.sub(6)),
            player.q.atomStrength.pow(player.q.atoms.sub(7)),
        ]
        player.q.intervalMul = new Decimal(0.8)
        player.q.buy10Mul = new Decimal(2)
        player.q.combustorPower = new Decimal(1.1)
        player.q.intensityMul = new Decimal(1.50)
        player.q.until10 = player.q.until10 ?? true
        player.q.intervalCap = new Decimal(10)

        player.q.commonGeneratorMultiplier = new Decimal(1)
        ...
        for (const id of [0, 1, 2, 3, 4, 5, 6]) {
            ...
            setBuyableAmount("q", "11" + (id), 
                ...
                .mul(player.q.commonGeneratorMultiplier)
                .mul(buyableEffect("q", "11" + (id + 1)))
                .mul(diff))
            )
            gen[id].intensity = gen[id].intensity.mul(player.q.atomMult[id])
        }

        player.q.lastTick = diff
        player.points = player.points.plus(buyableEffect("q", 110).mul(Decimal.div(1000, player.q.generators[0].interval.max(player.q.intervalCap))).mul(player.q.generators[0].intensity).mul(player.q.commonGeneratorMultiplier).mul(diff))
    },
    ...
    buyables: (function() {
        ...
        for (const id of [0, 1, 2, 3, 4, 5, 6, 7]) {
            buyables["10" + id] = {
                ...
                buy() {
                    if (this.canPhase() && player.q.generators[id].interval.lte(player.q.intervalCap)) {
                ...
                buyMax() {
                    ...
                    if (!this.canPhase() && player.q.generators[id].interval.gt(player.q.intervalCap)) {
            ...
            buyables["11" + id] = {
                ...
                buyMax() {
                    ...
                    let postScale = player.points.div(10).div(new Decimal(costMults[id])).log(scale[id]).sub(u308).max(0).root(2).plus(u308).floor().sub(u308.sub(1))
                    let amt = preScale
                    let x = player.q.generators[id].purchased
                    let _a = player.points.gte(this.cost(0, amt).mul(10)) ? 1 : 0

                    if (preScale.eq(u308)) amt = preScale.plus(postScale.max(1))
            ...
            buyables["12" + id] = {
                cost(x) {
                    let u308 = new Decimal("1.8e308").div(10000).div(new Decimal(costMults[id])).log(intensityScale[id]).floor()
                    if (x.gte(u308)) x = x.sub(u308).pow(2).plus(u308)
                ...
                display() {
                    return `
                        ...
                        <br>→ ×${format(player.q.generators[id].intensity.mul(player.q.intensityMul))} Output Costs: ${format(this.cost())} Quarks
                    `
                },
                canAfford() {
                    return player.points.gte(this.cost())
                ...
                buyMax() {
                    let x = getBuyableAmount(this.layer, this.id)
                    let u308 = new Decimal("1.8e308").div(10000).div(new Decimal(costMults[id])).log(intensityScale[id]).floor()
                    let preScale = player.points.div(10000).div(new Decimal(costMults[id])).log(intensityScale[id]).floor().plus(1).min(u308)
                    let postScale = player.points.div(10000).div(new Decimal(costMults[id])).log(intensityScale[id]).max(u308).sub(u308).root(2).plus(u308).floor().sub(u308.sub(1))
                    let amt = preScale
                    if (preScale.eq(u308)) amt = preScale.plus(postScale)
                ...
                effect() {
                    let x = getBuyableAmount(this.layer, this.id)
                    return player.q.intensityMul.pow(x).mul(buyableEffect("q", 11))
                },
            }
        }
        ...
        buyables["11"] = {
            cost(x) {
                let u308 = new Decimal("1.8e308").div(10000).log(100)
                if (x.gte(u308)) x = x.sub(u308).pow(2).plus(u308)
            ...
            display() {
                return `
                    <h2>Accelerators</h2>
                    Multiply all Generator Intensities by ×1.100:
                    → ${format(buyableEffect("q", 11).mul(player.q.combustorPower), 3)} Costs: ${format(this.cost())}
                `
            },
            canAfford() {
                return player.points.gte(this.cost())
            ...
            buyMax() {
                let u308 = new Decimal("1.8e308").div(10000).log(100).floor()
                let preScale = player.points.div(10000).log(100).ceil().min(u308)
                let postScale = player.points.div(10000).log(100).max(u308).sub(u308).root(2).ceil().max(0)
                let amt = preScale
                    if (preScale.eq(u308)) amt = preScale.plus(postScale)
                    setBuyableAmount(this.layer, this.id, amt)
    ...
    clickables: (function() {
        ...
        clickables["101"] = {
            ...
            display() {
                let phrase = ""
                return `
                    Unlock the next Generator; Multiply all Generators by ×${format(player.q.atomStrength)}
            ...
            content: [
                ...
                ["column",
                    [
                        ...
                        ["display-text",
                            function() { 
                                let mult = player.q.generators[0].purchased.div(10).floor().pow_base(player.q.buy10Mul)
                                    .mul(player.q.generators[0].intensity)
                                    .mul(player.q.commonGeneratorMultiplier)
                ...
                ["column",
                    [
                        ...
                        ["display-text",
                            function() { 
                                let mult = player.q.generators[1].purchased.div(10).floor().pow_base(player.q.buy10Mul)
                                    .mul(player.q.generators[1].intensity)
                                    .mul(player.q.commonGeneratorMultiplier)
                ...
                ["column",
                    [
                        ...
                        ["display-text",
                            function() { 
                                let mult = player.q.generators[2].purchased.div(10).floor().pow_base(player.q.buy10Mul)
                                    .mul(player.q.generators[2].intensity)
                                    .mul(player.q.commonGeneratorMultiplier)
                ...
                ["column",
                    [
                        ...
                        ["display-text",
                            function() { 
                                let mult = player.q.generators[3].purchased.div(10).floor().pow_base(player.q.buy10Mul)
                                    .mul(player.q.generators[3].intensity)
                                    .mul(player.q.commonGeneratorMultiplier)
                ...
                () => player.q.generators[4].unlocked ? ["column",
                    [
                        ...
                        ["display-text", `<h5>×${format(player.q.generators[4].purchased.div(10).floor().pow_base(player.q.buy10Mul).mul(player.q.generators[4].intensity).mul(player.q.commonGeneratorMultiplier))} | ${format(player.q.generators[4].purchased)}`],
                ...
                () => player.q.generators[5].unlocked ? ["column",
                    [
                        ...
                        ["display-text", `<h5>×${format(player.q.generators[5].purchased.div(10).floor().pow_base(player.q.buy10Mul).mul(player.q.generators[5].intensity).mul(player.q.commonGeneratorMultiplier))} | ${format(player.q.generators[5].purchased)}`],
                ...
                () => player.q.generators[6].unlocked ? ["column",
                    [
                        ...
                        ["display-text", `<h5>×${format(player.q.generators[6].purchased.div(10).floor().pow_base(player.q.buy10Mul).mul(player.q.generators[6].intensity).mul(player.q.commonGeneratorMultiplier))} | ${format(player.q.generators[6].purchased)}`],
                ...
                () => player.q.generators[7].unlocked ? ["column",
                    [
                        ...
                        ["display-text", `<h5>×${format(player.q.generators[7].purchased.div(10).floor().pow_base(player.q.buy10Mul).mul(player.q.generators[7].intensity).mul(player.q.commonGeneratorMultiplier))} | ${format(player.q.generators[7].purchased)}`],
        ...
        "Manifold Generators": {
            content: [
                ["layer-proxy", ["m", [
                    ["manifold-column", 
                        [
                            ["display-text", "<h3>1st Manifold Generator</h3>"],
                            ["row", 
                                [
                                    ["display-text", function() { return `<h3 style="position:absolute; left:25%">x Purchases | ×1` }],
                                    ["buyable", [100]],
                                ]
                            ]
                        ]
